...
-(NSMutableArray *) springAnimationValues:(id)fromValue toValue:(id)toValue usingSpringWithDamping:(CGFloat)damping initialSpringVelocity:(CGFloat)velocity duration:(CGFloat)duration{
    ...
    if (strcmp([fromValue objCType], @encode(CGPoint)) == 0) {
        CGFloat diffX = [toValue CGPointValue].x - [fromValue CGPointValue].x;
        CGFloat diffY = [toValue CGPointValue].y - [fromValue CGPointValue].y;
        for (NSInteger frame = 0; frame<numOfFrames; frame++) {
            
            CGFloat x = (CGFloat)frame / (CGFloat)numOfFrames;
            CGFloat valueX = [toValue CGPointValue].x - diffX * (pow(M_E, -damping * x) * cos(velocity * x)); // y = 1-e^{-5x} * cos(30x)
            CGFloat valueY = [toValue CGPointValue].y - diffY * (pow(M_E, -damping * x) * cos(velocity * x)); // y = 1-e^{-5x} * cos(30x)
            CGPoint point = CGPointMake(valueX, valueY);
            values[frame] = [NSValue valueWithCGPoint:point];
        }
    }
    else
    {
